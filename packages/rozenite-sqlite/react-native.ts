/**
 * React Native DevTools Plugin Entry Point
 *
 * This file serves as the main entry point for your DevTools plugin in the React Native environment.
 * You have full access to all React Native APIs and can integrate with your app's functionality.
 *
 * To communicate with the DevTools panel, use the `@rozenite/plugin-bridge` package
 * which provides a reliable communication channel between your plugin and the DevTools interface.
 */

// export * from './src/hooks/useRozeniteSQLite';

export default function setupPlugin(
  client
) {
  // Handle messages from DevTools panels with full type safety
//   client.onMessage('panel-mounted', (data) => {
//     // Access React Native APIs
//     // Send type-safe response
//     console.log("Panel mounted with data:", data);
//     client.send('panel-mounted', {
//       id: 'user-123',
//       name: 'John Doe',
//       email: 'john@example.com',
//     });
//   });

  client.onMessage('add-new-row', (payload) => {
    console.log("Add new row message received with payload:", payload);
    // Here you would typically trigger a UI in your app to add a new row to the specified database and table
    // For example, you could use an event emitter or a global state management solution to notify the relevant component
  });
}